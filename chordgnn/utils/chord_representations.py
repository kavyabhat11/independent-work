...
def time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=False, version="v1.0.0"):
    ...
    if "ts_beats" in df.columns and len(df) > 0:
        ts_value = df["ts_beats"].iloc[0]
        time_signature = int(ts_value)
        print(f"DEBUG: Read ts_beats={time_signature} from {time_divided_tsv_path}")
    else:
        ...
        print(f"DEBUG: Used fallback ts={time_signature} (no ts_beats column) for {time_divided_tsv_path}")
